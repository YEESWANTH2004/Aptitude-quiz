...
export default function QuizPage() {
  ...
  useEffect(() => {
    if (!attempt) {
      api.get(`/attempts/${attemptId}/result`).catch(() => navigate('/dashboard'));
    }
    setLoading(false);
  }, [attempt, attemptId, navigate]);
  ...
  // ── SOCKET — listen for admin force-terminate ─────────────────────────────
  useEffect(() => {
    ...
    socket.on('force-terminated', (data) => {
      if (terminatedRef.current) return;
      terminatedRef.current = true;
      setTerminated(true);
      toast.error(data.message || 'Your session was terminated by admin.');
      setTimeout(() => {
        navigate('/terminated', {
          replace: true,
          state: { reason: 'ADMIN_TERMINATED', attemptId },
        });
      }, 1500);
    });
  ...
  useEffect(() => {
    ...
    const handleBlur = () => {
      setTimeout(() => {
        if (document.visibilityState === 'hidden') handleViolation('WINDOW_BLUR');
      }, 300);
    };
      ...
      <div className="flex flex-1 overflow-hidden">
        ...
        <aside className="hidden lg:flex flex-col w-56 border-r border-surface-border bg-surface-card p-4 overflow-y-auto">
          ...
          <div className="mt-auto pt-4 space-y-1.5 text-xs text-gray-500">
            ...
            <div className="flex items-center gap-2"><div className="w-3 h-3 rounded bg-surface-muted" /> Unanswered</div>
          </div>
        </aside>
        ...
        <main className="flex-1 overflow-y-auto p-4 lg:p-8">
          <div className="max-w-2xl mx-auto">
            ...
            <div className="card p-6 lg:p-8 mb-6 animate-fade-in" key={currentQ?._id}>
              ...
              <QuestionCard question={currentQ} answer={currentAnswer} onAnswer={handleAnswer} />
            </div>
