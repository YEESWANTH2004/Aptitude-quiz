...
import Layout from '../../components/common/Layout';
import api from '../../utils/api';
import { ArrowLeft, Copy, CheckCircle, XCircle, Clock, PlayCircle, RefreshCw, ShieldX } from 'lucide-react';
import toast from 'react-hot-toast';
import { formatDistanceToNow } from 'date-fns';
...
const statusConfig = {
  SUBMITTED:   { label: 'Submitted',   icon: CheckCircle, color: 'text-emerald-400', cls: 'status-submitted' },
  TERMINATED:  { label: 'Terminated',  icon: XCircle,     color: 'text-red-400',     cls: 'status-terminated' },
  TIMED_OUT:   { label: 'Timed Out',   icon: Clock,       color: 'text-amber-400',   cls: 'status-timed_out' },
  IN_PROGRESS: { label: 'Live',        icon: PlayCircle,  color: 'text-brand-400',   cls: 'status-in_progress' },
};
...
export default function SessionDetailPage() {
  ...
  const [attempts, setAttempts] = useState([]);
  const [loading, setLoading] = useState(true);
  const [terminating, setTerminating] = useState(null); // attemptId being terminated
  ...
  useEffect(() => {
    ...
    // Live: new attempt started
    socket.on('attempt-started', (data) => {
      toast.success(`🟢 ${data.userName} just joined the quiz!`);
      fetchData(); // Refresh list
    });

    // Live: attempt submitted
    socket.on('attempt-submitted', (data) => {
      toast.success(`✅ A candidate submitted their quiz`);
      setAttempts(prev => prev.map(a =>
        a._id === data.attemptId
          ? { ...a, status: 'SUBMITTED', score: data.score }
          : a
      ));
    });

    // Live: attempt terminated (by candidate tab switch OR admin)
    socket.on('attempt-terminated', (data) => {
      setAttempts(prev => prev.map(a =>
        a._id === data.attemptId
          ? { ...a, status: 'TERMINATED' }
          : a
  ...
  // Admin force-terminate a candidate
  const handleAdminTerminate = async (attempt) => {
    const confirmed = window.confirm(
      `Terminate ${attempt.user?.name}'s quiz session? This cannot be undone.`
    );
    if (!confirmed) return;

    setTerminating(attempt._id);
    try {
      await api.post(`/attempts/${attempt._id}/admin-terminate`);
      toast.success(`${attempt.user?.name}'s session terminated`);
      // Update locally immediately
      setAttempts(prev => prev.map(a =>
        a._id === attempt._id ? { ...a, status: 'TERMINATED' } : a
      ));
    } catch (err) {
      ...
      <div className="max-w-5xl mx-auto">
        ...
        {inProgress.length > 0 && (
          <div className="card border-brand-500/20 bg-brand-500/5 mb-6">
            <div className="p-5 border-b border-brand-500/20 flex items-center gap-2">
              <span className="w-2 h-2 bg-brand-400 rounded-full animate-pulse" />
              <h2 className="font-display text-base font-semibold text-brand-300">
                Live Now — {inProgress.length} candidate(s) attending
              </h2>
            </div>
            <div className="divide-y divide-brand-500/10">
              {inProgress.map(a => (
                <div key={a._id} className="px-5 py-4 flex items-center justify-between gap-4">
                  <div>
                    <p className="font-medium text-white">{a.user?.name}</p>
                    <p className="text-xs text-gray-500">{a.user?.email}</p>
                  </div>
                  <div className="flex items-center gap-3">
                    <span className="text-xs text-brand-400">
                      {a.answers?.length || 0}/{a.totalQuestions} answered
                    </span>
                    <button
                      onClick={() => handleAdminTerminate(a)}
                      disabled={terminating === a._id}
                      className="flex items-center gap-1.5 px-3 py-1.5 bg-red-500/15 hover:bg-red-500/25 border border-red-500/30 text-red-400 text-xs font-semibold rounded-lg transition-all disabled:opacity-50"
                    >
                      {terminating === a._id
                        ? <span className="w-3 h-3 border border-red-400 border-t-transparent rounded-full animate-spin" />
                        : <ShieldX className="w-3 h-3" />
                      }
                      {terminating === a._id ? 'Terminating...' : 'Terminate'}
                    </button>
                  </div>
                </div>
              ))}
        ...
        {/* All attempts table */}
        <div className="flex items-center justify-between mb-4">
          <h2 className="font-display text-lg font-semibold text-white">All Attempts</h2>
          <button onClick={fetchData} className="flex items-center gap-2 text-sm text-gray-500 hover:text-white transition-colors">
            <RefreshCw className="w-4 h-4" /> Refresh
          </button>
        </div>
                          ...
                          <div className="flex items-center gap-2">
                            ...
                            {a.status === 'IN_PROGRESS' && (
                              <button
                                onClick={() => handleAdminTerminate(a)}
                                disabled={terminating === a._id}
                                className="flex items-center gap-1 px-2 py-1 bg-red-500/15 hover:bg-red-500/25 border border-red-500/30 text-red-400 text-xs font-semibold rounded-lg transition-all disabled:opacity-50"
                              >
                                <ShieldX className="w-3 h-3" />
                                Terminate
                              </button>
